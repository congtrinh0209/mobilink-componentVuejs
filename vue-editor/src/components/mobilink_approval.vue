        ...
        <v-card-text class="px-0 pb-0">
          ...
          <div v-for="(item, index) in approvalListItems" v-bind:key="index" class="mb-4">
            <div class="list__tile">
              <div class="avatar list__tile__avatar" style="height: 48px;width: 48px;"><img src="https://vuetifyjs.com/static/doc-images/lists/1.jpg"></div>
              <div class="list__tile__content">
                <strong>{{item.userName}}</strong>
                <div class="text-xs-center" v-if="approvalEditableIndex !== index">
                  {{item.subject}}
                  <v-chip v-if="item.state === 0">Đang trình duyệt</v-chip>
                ...
                <v-btn v-on:click.native="approvalEdit(item, index)" flat icon class="py-0 px-0"><v-icon>edit</v-icon></v-btn>
                <v-btn flat icon color="red darken-3" class="py-0 px-0" v-on:click.native="approvalRemove(item, index)" ><v-icon>clear</v-icon></v-btn>
              </div>
            </div>
...
.card--approval .btn--flat.btn--icon.red--text {
    right: 0px;
}
</style>
