    ...
    export default {
        ...
        computed: {
            eventStyle () {
                ...
                if (mm > this.start && mm < this.end+1) {
                    eventStyle.backgroundColor = '#c7e6fd'
                } else if (mm >= this.end+1) {
            ...
            showTitle() {
                return (this.date.day() == this.firstDay || this.start.isSame(this.date, 'day') || ( this.event.isShow && this.event.beSeted));
            },
            start () {
