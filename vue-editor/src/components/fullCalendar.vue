    ...
    <div class="comp-full-calendar">
      ...
      <div class="full-calendar-body">
        ...
        <div class="dates" ref="dates">
          ...
          <v-dialog v-model="showMore" persistent max-width="900" transition="fade-transition">
                ...
                <v-card-text style="padding: 0;padding-top: 50px;">
                  <ul class="ul-with-border ul-with-right-icon ul-with-left-icon">
                    <li v-for="(item, index) in selectDay.events" :key="index"
                      v-show="item.isShow" @click="eventClick(event, $event)">
                            ...
                            <v-card>
                              <v-card-text>
                                <div class="layout row wrap">
                                  <v-flex xs6>{{item.cellIndex}}. {{ item.subject }} | {{ item.activityStatus}}</v-flex>
                                </div>
                                <div>
                                  <v-icon>schedule</v-icon>
                                    <span>{{getMoment(item.startDate).format('HH:mm DD/MM/YYYY')}}</span>
          ...
          <div class="detail-event" ref="eventHover" v-show="showDetail"
            ...
            <div class="detail-event-body-">
              ...
              <v-card>
								<v-card-text>
                  <div>
                    {{selectEvent.cellIndex}}. {{ selectEvent.subject }} | {{ selectEvent.activityStatus}} 
                  </div>
                  <div>
                    <v-icon>schedule</v-icon>
                    <span>{{getMoment(selectEvent.startDate).format('HH:mm DD/MM/YYYY')}}</span>
  ...
  export default {
    ...
    methods : {
      ...
      slotEvents (date) {
        ...
        for (let i = 0;i < thisDayEvents.length;i++) {
          if (thisDayEvents[thisDayEvents.length-1].isShow &&
              (thisDayEvents.length > eventLim) &&
              !thisDayEvents[i].isShow) {
                
                for(let key in thisDayEvents[thisDayEvents.length-1]) {
                  if (key === 'cellIndex') {
                    thisDayEvents[i].beSeted = true
                    continue
                  }
                  thisDayEvents[i][key] = thisDayEvents[thisDayEvents.length-1][key]
                }
            thisDayEvents.splice(thisDayEvents.length-1, 1)
          }
        }
      ...
      computePos (target) {
        ...
        console.log(pageRect.right, posRight)
        posLeft = pageRect.right > posRight?posLeft:pageRect.right-eventHoverRect.width-pageRect.left
        return {
          left : posLeft,
