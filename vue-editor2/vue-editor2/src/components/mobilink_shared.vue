        ...
        <v-dialog v-model="sharedDialog" persistent max-width="900" transition="fade-transition">
            <v-card style="min-height: 500px;" :class="{'mobilink-shared-root editable-wrap editable-wrap-before-select': permission !== 'write' && permission !== 'owner'}">
                ...
                <v-card-text style="padding-top: 35px;">
                    ...
                    <div v-if="permission === 'write' || permission === 'owner'">
                      <v-radio-group @change="sharedVisibilityChangeValue" v-model="sharedVisibility" :input-value="sharedVisibility" :mandatory="false">
                        <v-radio v-if="share_for !== 'docfile'" label="Công bố ra ngoài" value="0"></v-radio>
                        <v-radio label="Công khai cơ quan" value="1"></v-radio>
                        <v-radio label="Nội bộ nhóm" value="2"></v-radio>
                      </v-radio-group>
                    </div>
                    <div style="margin-top: 25px;" v-else>
                      <label style="display: block;" v-if="sharedVisibility === '1'">Công khai cơ quan</label>
                      <label style="display: block;" v-else-if="sharedVisibility === '2'">Nội bộ nhóm</label>
                      <label style="display: block;" v-else>Công bố ra ngoài</label>
                    </div>
                ...
                <v-card-actions v-if="permission === 'write' || permission === 'owner'">
                    <v-spacer></v-spacer>
                    <v-btn flat @click.native="sharedDialog = false"><v-icon>close</v-icon> &nbsp; Quay lại </v-btn> 
                    <v-btn flat v-on:click.native="sharedDialogSubmit()"
...
export default {
  ...
  methods: {
    ...
    sharedDialogFunc: function () {
      var vm = this
      if (vm.share_for === 'docfile') {
        vm.sharedVisibility = '2'
      }
      ...
      if (vm.visibilities_api != null) {
        ...
        .then(function (response) {
          var serializable = response.data
          vm.sharedVisibility = serializable.state + '';
        })
        .catch(function (error) {
          console.log(error)
